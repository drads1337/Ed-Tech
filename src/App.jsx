...
function VoiceChat({ scenarioTitle, scenarioGoal, aiPersona, patientType, language, onMotionChange, onEmotionChange }) {
  ...
  // Keep ref current so the MediaRecorder onstop closure always calls the latest version
  ...
  const startListening = useCallback(async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        if (blob.size < 500) { setStatus('idle'); onMotionChange?.('idle'); return; }
  ...
  const handleMicButton = useCallback(() => {
    if (status === 'listening') {
      stopListening(); // stop recording → onstop fires → transcribe → sendUserMessage
    } else if (status === 'idle') {
      startListening();
    }
